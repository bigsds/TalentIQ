...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (isNaN(mandatId)) return NextResponse.json({ error: 'ID invalide' }, { status: 400 })
  ...
  const [
    ...
    prisma.shortlist.findMany({
      where: { mandatId },
      orderBy: [{ rang: 'asc' }, { createdAt: 'asc' }],
  ...
  for (const g of genreData) genre[g.genre] = g._count
  ...
  return NextResponse.json({
    ...
    shortlists: shortlists.map((s, i) => ({
      id: s.id,
      rang: s.rang ?? i + 1,
      noteHg: s.noteHg,
      candidature: {
        id: s.candidature.id,
        nom: s.candidature.nom,
        prenom: s.candidature.prenom,
        score: s.candidature.score,
        recommendation: s.candidature.recommendation,
        pointsForts: s.candidature.pointsForts,
        anneesExperience: s.candidature.anneesExperience,
        niveauFormation: s.candidature.niveauFormation,
      },
    })),
  })
}
